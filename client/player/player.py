...
class Player:
    ...
    def __init__(self, piece_id, username):
        logger.info(f"Player initialized with piece_id {piece_id} and username {username}")
        self.piece_id = piece_id
        self.username = username

    def make_move(self, board: Board, y: int, x: int) -> bool:
        if board.get(y, x) == 0:
            # logger.info(f"Player {self.piece_id} made a move at ({y}, {x}).")
            board.set(y, x, self.piece_id)
            return True
        logger.info(f"Player {self.piece_id} failed to make a move at ({y}, {x}): Cell is already occupied.")  
        return False
...
class OnlinePlayer(Player):
    ...
    def make_move(self, board, y, x):
        ...
        # logger.info(f"OnlinePlayer {self.piece_id} thực hiện nước đi tại ({y}, {x}) qua mạng.")
