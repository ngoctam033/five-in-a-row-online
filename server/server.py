...
class WebSocketServer:
			...
			async for message in websocket:
				...
				try:
					...
					if data.get("type") == "create_room":
						...
						# logging.info(f"Room creation response: {response}")
					if data.get("type") == "get_online_players":
						response = self.get_online_players(user_name=data.get("player"))
						# logging.info(f"Online players for {data.get('player')}: {response}")
	...
	async def get_opponent_move(self, data):
		...
		if current_player != player_name:
			logging.warning(f"Player {player_name} tried to move, but it's not their turn.")
			return {"type": "error", "message": "Chưa đến lượt của bạn."}

		logging.info(f"Current turn {current_player}")
		# Tìm đối thủ
		opponent = None
		...
		try:
			...
			# logging.info(f"Sent opponent move to {opponent.name}: {opponent_move_data}")
			...
			logging.info(f"Current turn {room.current_player()}")
	...
	def create_room_with_players(self, player1_name, player2_name):
		...
		if not player1 or not player2:
			logging.warning(f"Cannot create room: one or both players not found (player1: {player1}, player2: {player2})")
			return None
		...
		if room1 or room2:
			logging.warning(f"Cannot create room: one or both players are already in a room (player1: {player1_name}, player2: {player2_name})")
			return None
